...
def get_skin_data(uuid):
    try:
        # 更換為最新、最穩定的全球造型 API 路徑，解決未知造型問題
...
async def shop(interaction: discord.Interaction, entitlement_token: str = None, access_token: str = None, user_id: str = None):
    ...
    # 如果沒輸入 Token，跳出全新、模擬啟動器且絕對不會 404 的安全通道指南
    if not entitlement_token or not access_token or not user_id:
        embed = discord.Embed(
            ...
            description="為了帳號安全，本群不收集密碼。請用以下純官方、保證不壞的方式取得金鑰：\n\n"
                        ...
                        "   [👉 點我前往全新官方跳轉網址](https://auth.riotgames.com/authorize?client_id=riot-client&redirect_uri=http%3A%2F%2Flocalhost%2Fredirect&response_type=token%20id_token&scope=openid%20link%20accounts)\n\n"
